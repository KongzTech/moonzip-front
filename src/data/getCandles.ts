import {getProjectCandles, GetProjectCandlesRequest} from "@/api/project/getProjectCandles.ts";
...
export async function getCandles(projectId: string, options: { period: string; time: number; n: number }) {
    const ENV_STAGE = import.meta.env.VITE_ENV_STAGE;
    if (ENV_STAGE === "test") {
        return oneMinCandles.filter((candle) => candle.time <= options.time).slice(0, options.n);
    }
    const request: GetProjectCandlesRequest = {
        projectId,
        timeRange: options.period,
        time: options.time,
    }
    const result = await getProjectCandles(request);
